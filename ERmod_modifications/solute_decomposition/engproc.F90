...
contains
  ...
  subroutine enginit
    use engmain, only: numtype, nummol, engdiv, corrcal, selfcal, slttype, &
         numgrp, group_decomposition, &                              ! MotifD
         moltype, sluvid, &
    ...
    !
    call group_decomposition                                         ! MotifD
    numslv = numslv * numgrp                                         ! MotifD
    ...
    do pti = 0, numslv
       ...
       if(ecprread == YES) then
          ...
          do i = 1, large
             ...
             if(q > 0) then                                          ! MotifD
                backspace(ecdinfo_io)                                ! MotifD
                read(ecdinfo_io, *, end = 3109) q_u, q_v             ! MotifD
     ! ordering of (u1, v1), (u2, v1), ... (u1, vn), (u2, vn) ...    ! MotifD
                q = q_u + (q_v - 1) * numgrp                         ! MotifD
     ! ordering of (u1, v1), (u1, v2), ... (un, v1), (un, v2) ...    ! MotifD
             !  q = (q_u - 1) * (numslv / numgrp) + q_v              ! MotifD
             endif                                                   ! MotifD
             if(q == pti) then
                ...
                if(pti == 0) then         ! solute self-energy
                   ...
                   read(ecdinfo_io, *) q_u, q_v, ecpmrd(1:9), pecore ! MotifD
                   ecdmax = ecpmrd(9)
...
3209      continue
  ...
  subroutine engconst(stnum)
    use engmain, only: nummol, skpcnf, slttype, sluvid, &
                       ...
                       io_flcuv, &
                       numgrp, cntgrp, group_decomposition, &        ! MotifD
    ...
    if(myrank < nactiveproc) then
       ...
       do cntdst = 1, maxdst
          do cntgrp = 0, numgrp                                      ! MotifD
          call group_decomposition(cntdst)                           ! MotifD
...
751          format(i5, 9999f15.5)
  ...
  subroutine update_histogram(stat_weight_solute, uvengy)
    ...
    integer, allocatable, save :: insdst(:)                          ! MotifD
    ...
    do k = 1, slvmax
       ...
       pti = uvspec(i)
    ! ordering of (u1, v1), (u2, v1), ... (u1, vn), (u2, vn) ...     ! MotifD
       pti = cntgrp + (pti - 1) * numgrp                             ! MotifD
    ...
    if(slttype == SLT_SOLN) then
       do i = 1, numslv / numgrp                                     ! MotifD
          pti = cntgrp + (i - 1) * numgrp                            ! MotifD
          slnuv(pti) = slnuv(pti) + flceng(pti, cntdst) * engnmfc    ! MotifD
       enddo                                                         ! MotifD
    endif

    if(cntgrp == numgrp) then                                        ! MotifD
    do iduv = 1, ermax
       k = insdst(iduv)
       if(k == 0) cycle
