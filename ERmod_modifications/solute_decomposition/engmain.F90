...
!
  ...
  integer                            :: numgrp, cntgrp      ! MotifD
  integer, dimension(:), allocatable :: int_motif           ! MotifD
  real, dimension(:), allocatable    :: charge_sltmol       ! MotifD
...
contains 
  ...
  ! start of intramolcular decomposition of solute for MotifD
  subroutine group_decomposition(cntdst)
    implicit none
    integer, intent(in), optional :: cntdst
    character(len=*), parameter :: group_file = 'SltAtomGroup'
    integer, parameter :: atomgrp_io = 38
    logical, save :: first_time = .true.
    integer :: tagslt, stmax, ptb, pte, i, cntatm, ioerr_row, ioerr_column
    integer, dimension(:), allocatable :: atom_list
    character(len=4096) :: read_line

    select case(slttype)
    case(SLT_SOLN)
       if(present(cntdst)) then
          if(cntdst == 0) then
             tagslt = sltlist(1)
          else
    ...
    if( first_time ) then
       allocate( int_motif(stmax), charge_sltmol(stmax) )
       int_motif(:) = 0
       charge_sltmol(1:stmax) = charge(ptb:pte)

       allocate( atom_list(stmax) )
       numgrp = 0
       open(unit = atomgrp_io, file = group_file, status = 'old')
       do
          read(atomgrp_io, *, iostat = ioerr_row) i
          if(ioerr_row == 0) then
             numgrp = numgrp + 1
             backspace(atomgrp_io)
             read(atomgrp_io, '(A)') read_line
             cntatm = 1
             do
                atom_list(:) = 0
                read(read_line, *, iostat = ioerr_column) atom_list(1:cntatm)
                if(ioerr_column == 0) then
                   cntatm = cntatm + 1
                   cycle
                else
                   cntatm = cntatm - 1
             ...
             enddo
             do i = 1, stmax
                if(count( mask = (atom_list(:) == i) ) > 1) then
                   call incorrect_SltAtomGroup
                endif
                if(any(atom_list(:) == i)) then
                   if(int_motif(i) == 0) then
                      int_motif(i) = numgrp
                   else
                      call incorrect_SltAtomGroup
                   endif
       ...
       do i = 1, numgrp
          if(count( mask = (int_motif(:) == i) ) == 0) then
             call incorrect_SltAtomGroup
          endif
       enddo
       if(any(int_motif(:) == 0)) call incorrect_SltAtomGroup

       first_time = .false.
       return
    endif

    charge(ptb:pte) = charge_sltmol(1:stmax)
    if(cntgrp > 0) then
       where( int_motif(1:stmax) /= cntgrp ) charge(ptb:pte) = 0
    endif

  contains
    subroutine incorrect_SltAtomGroup
      write(stdout, '(A)') "Incorrect SltAtomGroup"
      stop
    end subroutine incorrect_SltAtomGroup
  end subroutine group_decomposition

  subroutine get_slt_internal_id(ua,i)
    integer, intent(in) :: ua
    integer, intent(out) :: i
    integer :: k
    k = belong_to(ua)
    i = ua - mol_begin_index(k) + 1
  end subroutine get_slt_internal_id
  ! end of intramolcular decomposition of solute for MotifD
end module engmain
